export default class CameraMoveScript  {
    ...
    protected owner:Laya.BaseCamera;
     ...
     * @inheritDoc
     */
    public onInit(owner:Laya.Camera,):void {
        ...
        var scene:Laya.Scene3D = owner.scene;
        owner.active = false;
        this.owner = owner;
        this.camera = owner.clone() as Laya.Camera;
        scene.addChildAt(this.camera,0);
    }
    public onRemove(){
        if( this.camera){
    ...
    public  onUpdate():void {
        ...
        if(Laya.KeyBoardManager.hasKeyDown(Laya.Keyboard.CONTROL)){
        ...
        if (!isNaN(this.lastMouseX) && !isNaN(this.lastMouseY) && this.isMouseDown) {
            var scene:Laya.Scene3D = this.camera.scene;
            // Laya.KeyBoardManager.hasKeyDown(87) && this.moveForward(-0.01 * elapsedTime);//W
            // Laya.KeyBoardManager.hasKeyDown(83) && this.moveForward(0.01 * elapsedTime);//S
            // Laya.KeyBoardManager.hasKeyDown(65) && this.moveRight(-0.01 * elapsedTime);//A
            // Laya.KeyBoardManager.hasKeyDown(68) && this.moveRight(0.01 * elapsedTime);//D
    ...
    protected mousWheel(e){
        if(Laya.KeyBoardManager.hasKeyDown(Laya.Keyboard.CONTROL))
           this.moveForward(e.delta*0.5);
    }
    /**
     * 向前移动。
     * @param distance 移动距离。
